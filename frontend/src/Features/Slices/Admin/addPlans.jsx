...
export const getFullPlans = createAsyncThunk("getInterview/Plans", async () => {
  try {
...
export const subscribedPlanEdit = createAsyncThunk("edit/Plans", async (values) => {
  try {
...
const subscriptionPlanSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(getFullPlans.fulfilled, (state, action) => {
      console.log(action);
      console.log('action');
      state.FullPlanDetails = action.payload?.data?.fullPlans
    });
